...
{% macro priority_reject_reason(col) %}
    case
        when {{ col }} is null or {{ col }} = '' then 'priority rỗng'
        when {{ col }} in ('0', '5', '-1') then 'priority số nguyên ngoài khoảng 1..4'
        else 'priority không quy đổi được (' || coalesce({{ col }}, 'NULL') || ')'
    end
{% endmacro %}
